@include('admin.layouts.headermodule')

<body class="hold-transition sidebar-mini zw_sidebar">
  <div class="wrapper">
    @include('admin.layouts.header')
    @include('admin.layouts.sidebar')

    <!-- Content Wrapper. Contains page content -->
    <div class="content-wrapper">
      <!-- Content Header (Page header) -->
      <section class="content-header">
        <div class="container-fluid">
          
          <div class="row mb-2">
           <div class="col-12 col-md-12 col-lg-6">
            <h1 class="poppins-semibold zw_46 zw_text_AF2245">
              <span class="mr-3"><img src="{{url('dist/img/zwaara_user.png')}}" alt="Zwaara User" class=" "></span>Patient Details</h1>
              <!-- <p class="poppins-regular zw_16 zw_text_111535 mt-2 mb-1">You have 10 Registration requests</p> -->
            </div>
          </div>
        </div><!-- /.container-fluid -->
      </section>
      <!-- Content Header (Page header) -->
      <section class="content-header">
        <div class="row mb-2">
          <div class="col-12 col-md-12 col-lg-6">
            <form class="zw_search_form ">
              <div class="input-group input-group-sm">
                <input type="text" name="search" class=" zw_search_input zw_16 poppins-regular" placeholder="Search by Name">
                <div class="input-group-append zw_input-group-append">
                  <button type="submit" class="btn zw_search_button zw_bg_ffffff">
                    <i class="fas fa-search"></i>
                  </button>
                </div>
              </div>
            </form>
          </div>
        </div>
        <div class="row">
          <div class="col-12">
            <div class="card zw_card">
              @if ($message = Session::get('success'))
              <div class="alert zw_alert_success">
                <p>{{ $message }}</p>
              </div>
              @endif
              <!-- /.card-header -->
              <div class="card-body table-responsive p-0">
               
                <!-- Add more buttons/links for other status as needed -->
                 <div class="row">
                  <div class="col-12">
                    <a class="btn zw_service_btn active" href="{{ route('patientregistration.index') }}">All</a>
                    <a class="btn zw_service_btn active" href="{{ route('patient.type', ['type' => 'Register']) }}">Register Patients</a>
                    <a class="btn zw_service_btn" href="{{ route('patient.type', ['type' => 'Non Register']) }}">Non Register Patients</a>
                    <a class="btn zw_service_btn" href="{{ route('patient.type', ['type' => 'Iqama']) }}">Iqama Patients</a>
                    <a class="btn zw_service_btn" href="{{ route('patient.status', ['status' => 0]) }}">Rejected Patients</a>
                  </div>
                </div>
                <table class="table table-hover text-nowrap zw_table">
                  <thead class="zw_bg_AF2245 zw_text_ffffff">
                    <tr>
                      <th class="poppins-medium zw_18">Date</th>
                      <th class="poppins-medium zw_18">Healthcare</th>
                      <th class="poppins-medium zw_18">First Name</th>
                      <th class="poppins-medium zw_18">Last Name</th>
                      <th class="poppins-medium zw_18">City</th>
                      <th class="poppins-medium zw_18">Email</th>
                      <th class="poppins-medium zw_18">Phone</th>
                      <th class="poppins-medium zw_18" colspan="3"></th>
                    </tr>
                  </thead>
                  <tbody>
                    @foreach ($patientregistration as $patientregistration)
                    <tr class="m-4">
                      <td class="poppins-regular zw_14 zw_text_333333">{{ $patientregistration->CreatedAt }}</td>
                      <td class="poppins-regular zw_14 zw_text_333333">{{ $patientregistration->Healthcare }}</td>
                      <td class="poppins-regular zw_14 zw_text_333333">{{ $patientregistration->Firstname }}</td>
                      <td class="poppins-regular zw_14 zw_text_333333">{{ $patientregistration->Lastname }}</td>
                      <td class="poppins-regular zw_14 zw_text_333333">{{ $patientregistration->City }}</td>
                      <td class="poppins-regular zw_14 zw_text_333333">{{ $patientregistration->Email }}</td>
                      <td class="poppins-regular zw_14 zw_text_333333">{{ $patientregistration->Phone }}</td>
                      <td class="poppins-regular zw_14 zw_text_333333" >
                        <form id="statusForm" class="statusForm">
                          @csrf
                          <input type="hidden" name="prid" id="prid" class="prid" value="{{ $patientregistration->id }}">
                          <input type="checkbox" data-toggle="toggle" data-size="sm" id="StatusUpdate" class="StatusUpdate" data-on="Approved" data-off="Rejected" {{ $patientregistration->Status ? 'checked' : '' }} >
                        </form>     
                      </td>
                      <td>
                        <a class="btn zw_text_AF2245 zw_a zw_24" href="{{ route('patientregistration.show',$patientregistration->id) }}"><i class="fa fa-edit" aria-hidden="true"></i></a>
                      </td>
                      <td>
                        <form action="{{ route('patientregistration.destroy',$patientregistration->id) }}" method="POST">
                          @csrf
                          @method('DELETE')
                          <button type="submit" class="btn zw_text_AF2245 zw_a zw_24"><i class="fas fa-trash"></i></button>
                        </form>
                      </td>
                    </tr>
                    @endforeach
                  </tbody>
                </table>
              </div>
              <!-- /.card-body -->
            </div>
            <!-- /.card -->
          </div>
        </div>
        <!-- /.row -->
      </section>
    </div>
    @include('admin.layouts.footer')
  </div>
    <script>
    $(document).ready(function() {
       $('.StatusUpdate').change(function() {
        var status = $(this).prop('checked');
        var prid = $(this).closest('.statusForm').find('.prid').val();
        
        $.ajax({
          url: "{{ route('prupdate.status') }}",
          method: 'POST',
          data: {
            _token: "{{ csrf_token() }}",
            id:  prid,
            status: status
          },
          success: function(response) {
            console.log('Status updated successfully');
                // You can add any additional logic here if needed
          },
          error: function(xhr, status, error) {
            console.error('Error updating status:', error);
          }
        });
      });
    });
  </script>
</body>
</html>